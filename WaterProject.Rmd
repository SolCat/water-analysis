...
<div style="text-align:justify">
Nous considérons enfin le jeu de données, où les observations sont passées en tant qu'individus supplémentaires. En effet, l'ACP est sensible à des valeurs qui peuvent varier dans les extrêmes. De plus, si une erreur survient dans les étapes entre la mesure des valeurs et leur analyse ( étalonnage des instruments, recopie manuelle des valeurs, autres ) il peut être interessant d'enlever des valeurs qui pourraient paraître erronées pour la construction de notre ACP.  
  
Il faut donc séparer les valeurs qui se démarquent trop. La fonction `boxplot.stats(x)$out` renvoie les valeurs au dessous et au dessus de 4 fois l'intervalle interquartile. Avec ce traitement appliqué à chaque variable, on peut récupérer la liste des individus aux valeurs extrêmes. On peut ensuite créer deux listes distinctes, qui seront nos individus actifs d'un côté et passifs de l'autre.  
  
```{r}
num_outliers = unique(unlist(apply(acp.variables, 2, function(x){which(x %in% boxplot.stats(x)$out)})))
outliers = acp.variables[num_outliers,]
without_outliers = acp.variables[setdiff(c(1:nrow(acp.variables)), num_outliers),]
```
  
Une fois le filtre sur les valeurs manquantes, puis extrêmes appliqué, on se retrouve avec seulement 31 individus. Cependant on peut quand même espérer une meilleure représentation.
...
#####<u>Analyse croisée des variables et observations</u>
